...
static int at_exec_sma_month(char *str)
{
	long_byte_s fram_val;
	long new_month_val = strtol(str, NULL, 0);
...
static int at_query_sma_month(void)
{
	snprintf(g_at_query_buf, ATQUERY_SIZE, "%ld", g_month_energy);
	return 0;
}

static int at_exec_sma_year(char *str)
{
	long_byte_s fram_val;
	long new_year_val = strtol(str, NULL, 0);
...
static int at_query_sma_year(void)
{
	snprintf(g_at_query_buf, ATQUERY_SIZE, "%ld", g_year_energy);
	return 0;
}
...
static atcmd_t g_at_cmd_list[] = {
	...
	{"+APPEUI", "Get/set the application EUI", at_query_appeui, at_exec_appeui, NULL},
	{"+APPKEY", "Get/set the application key", at_query_appkey, at_exec_appkey, NULL},
	{"+DEVEUI", "Get/set the device EUI", at_query_deveui, at_exec_deveui, NULL},
	{"+APPSKEY", "Get/set the application session key", at_query_appskey, at_exec_appskey, NULL},
	{"+NWKSKEY", "Get/set the network session key", at_query_nwkskey, at_exec_nwkskey, NULL},
	{"+DEVADDR", "Get/set the device address", at_query_devaddr, at_exec_devaddr, NULL},
	// Joining and sending data on LoRa network
	{"+CFM", "Get/set the confirm mode", at_query_confirm, at_exec_confirm, NULL},
	{"+JOIN", "Join network", at_query_join, at_exec_join, NULL},
	{"+NJS", "Get the join status", at_query_join_status, NULL, NULL},
	{"+NJM", "Get/set the network join mode", at_query_joinmode, at_exec_joinmode, NULL},
	{"+SENDINT", "Get/set the automatic send time", at_query_sendfreq, at_exec_sendfreq, NULL},
	// LoRa network management
	{"+ADR", "Get/set the adaptive data rate setting", at_query_adr, at_exec_adr, NULL},
	{"+CLASS", "Get/set the device class", at_query_class, at_exec_class, NULL},
	{"+DR", "Get/set the Tx DataRate=[0..7]", at_query_datarate, at_exec_datarate, NULL},
	{"+TXP", "Get/set the transmit power", at_query_txpower, at_exec_txpower, NULL},
	{"+BAND", "Get/set number corresponding to active regions", at_query_region, at_exec_region, NULL},
	{"+MASK", "Get/set channels mask", at_query_mask, at_exec_mask, NULL},
	// Status queries
	...
	{"+SMA", "Get/set SMA inverter IP address", at_query_sma, at_exec_sma, NULL},
	{"+SMON", "Get/set current month production so far", at_query_sma_month, at_exec_sma_month, NULL},
	{"+SYEAR", "Get/set current year production so far", at_query_sma_year, at_exec_sma_year, NULL},
	// RTC queries
