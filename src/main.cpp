...
void setup()
{
	...
	fram_val.val_32 = 0;
	read_rak15005(g_month_energy_addr, fram_val.val_8, 4);
	...
	fram_val.val_32 = 0;
	read_rak15005(g_year_energy_addr, fram_val.val_8, 4);
		...
		while (!isSuccess)
		{
			...
			if (isSuccess && (values[0] < 3000))
			{
				...
				if ((g_date_time.hour == 23) && (g_date_time.minute >= 50) && !daily_added)
				{
					...
					// // Send months production to today
					// g_solution_data.addEnergy(0x03, (float)g_month_energy / 1000.0);
					// // Send years production to today
